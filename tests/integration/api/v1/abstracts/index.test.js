...
describe('POST /api/v1/abstracts', () => {
    test('Creating an abstract without authentication', async () => {
        ...
        expect(responseBody.error).toBe(
            'User must be authenticated to submit abstracts',
        );
    });
    ...
    test('Creating an abstract with valid data', async () => {
        ...
        const abstractResponse = await fetch(
            'http://localhost:3000/api/v1/abstracts',
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                ...
                body: JSON.stringify({
                    ...
                    content:
                        'This paper explores the application of deep learning models for improving climate prediction accuracy.',
                    keywords: [
                        'deep learning',
                        'climate',
                        'prediction',
...
describe('GET /api/v1/abstracts', () => {
    ...
    test('Filtering abstracts by conference_id', async () => {
        const response = await fetch(
            'http://localhost:3000/api/v1/abstracts?conference_id=1',
        );
    ...
    test('Pagination with limit and offset', async () => {
        const response = await fetch(
            'http://localhost:3000/api/v1/abstracts?limit=5&offset=0',
        );
...
describe('PATCH /api/v1/abstracts/[id]', () => {
    test('Updating own abstract', async () => {
        ...
        const updateResponse = await fetch(
            `http://localhost:3000/api/v1/abstracts/${abstract.id}`,
            {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',

...
export default function ManageSpeakers() {
  ...
  const fetchData = async () => {
    try {
      ...
      const speakersResponse = await fetch(`/api/v1/conferences/${confData.id}/speakers`);
  ...
  const handleDelete = async (speakerId) => {
    ...
    try {
      const response = await fetch(`/api/v1/conferences/${conference.id}/speakers/${speakerId}`, {
        method: 'DELETE',
      });
                ...
                <div style={styles.speakerInfo}>
                  <div>
                    <h3 style={styles.speakerName}>
                      {speaker.name}
                      {speaker.is_keynote && <span style={styles.keynoteBadge}>Keynote</span>}
                    </h3>
                    ...
                    {speaker.bio && <p style={styles.speakerBio}>{speaker.bio. substring(0, 150)}...</p>}
                  </div>
                  <div style={styles.speakerActions}>
                    <button onClick={() => handleEdit(speaker)} style={styles.editButton}>
                      Edit
                    </button>
                    <button onClick={() => handleDelete(speaker.id)} style={styles.deleteButton}>
                      Delete
                    </button>
                  </div>
                </div>
        ...
        <div style={styles.modalOverlay} onClick={() => setShowModal(false)}>
          <div style={styles.modal} onClick={(e) => e. stopPropagation()}>
            <div style={styles.modalHeader}>
              <h2>{editingSpeaker ? 'Edit Speaker' : 'Add Speaker'}</h2>
            ...
            <form onSubmit={handleSubmit} style={styles.form}>
              ...
              <div style={styles.formRow}>
                <div style={styles.formGroup}>
                  <label style={styles.label}>Twitter Handle</label>
                  <input
                    type="text"
                ...
                <div style={styles.formGroup}>
                  <label style={styles.checkboxLabel}>
                    <input
                      ...
                      onChange={(e) => setFormData({ ...formData, is_keynote: e.target.checked })}
                    />
                    Keynote Speaker
                  </label>
                </div>

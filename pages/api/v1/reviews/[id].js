    ...
    try {
        ...
        if (!canUpdate) {
            throw new validator.UnauthorizedError(
                'User must be authenticated to update reviews',
            );
        }
                ...
                throw new validator.ValidationError([
                    {
                        ...
                        message:
                            'Recommendation must be one of: accept, reject, revise',
                    },
                ]);
            }
...
async function deleteReview(request, response, reviewId) {
    try {
        ...
        if (!canDelete) {
            throw new validator.UnauthorizedError(
                'User must be authenticated to delete reviews',
            );
        }

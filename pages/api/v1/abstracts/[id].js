...
async function updateAbstract(request, response, abstractId) {
    try {
        // Check authorization
        const canUpdate = await authorization.canRequest(
            'update:abstract',
            request,
        );
        if (!canUpdate) {
            throw new validator.UnauthorizedError(
                'User must be authenticated to update abstracts',
            );
        }
        ...
        if (
            existingAbstract.author_id !== currentUser.id &&
            !currentUser.features.includes('admin')
        ) {
            throw new validator.ForbiddenError(
                'You can only update your own abstracts',
            );
        }
        ...
        if (request.body.status !== undefined) {
                ...
                throw new validator.ValidationError([
                    {
                        field: 'status',
                        message:
                            'Status must be one of: submitted, under_review, accepted, rejected',
                    },
                ]);
            }
            ...
            if (!currentUser.features.includes('admin')) {
                throw new validator.ForbiddenError(
                    'Only admins can change abstract status',
                );
            }
...
async function deleteAbstract(request, response, abstractId) {
    try {
        // Check authorization
        const canDelete = await authorization.canRequest(
            'delete:abstract',
            request,
        );
        if (!canDelete) {
            throw new validator.UnauthorizedError(
                'User must be authenticated to delete abstracts',
            );
        }
        ...
        if (
            ...
            throw new validator.ForbiddenError(
                'You can only delete your own abstracts',
            );
        }

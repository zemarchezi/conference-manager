    ...
    try {
        ...
        if (!canUpdate) {
            throw new validator.UnauthorizedError(
                'User must be authenticated to update schedules',
            );
        }
        ...
        if (!currentUser.features.includes('admin')) {
            throw new validator.ForbiddenError(
                'Only admins can update schedule entries',
            );
        }
...
async function deleteSchedule(request, response, scheduleId) {
    try {
        ...
        if (!canDelete) {
            throw new validator.UnauthorizedError(
                'User must be authenticated to delete schedules',
            );
        }
        ...
        if (!currentUser.features.includes('admin')) {
            throw new validator.ForbiddenError(
                'Only admins can delete schedule entries',
            );
        }

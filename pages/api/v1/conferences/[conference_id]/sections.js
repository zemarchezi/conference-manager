...
export default async function handler(request, response) {
    ...
    try {
        ...
        switch (section) {
            case 'submissions':
                const submissions = await abstract.findAll(conferenceId, {
                    limit: 100,
                });
            ...
            case 'schedule':
                const scheduleData =
                    await schedule.findByConferenceId(conferenceId);
                return response
                    .status(200)
                    .json({ schedule: scheduleData || [] });
          ...
          `,
                ...
                return response
                    .status(200)
                    .json({ speakers: speakersResult.rows });

            default:
                return response.status(400).json({
                    error: 'Invalid section requested.  Use: submissions, schedule, or speakers',
                });
        }
    } catch (error) {
        ...
        return response
            .status(500)
            .json({ error: 'Failed to fetch section data' });
    }
}

    ...
    try {
        ...
        if (!currentUser) {
            return response.status(401).json({
                error: 'User must be authenticated to delete conferences',
            });
        }
        ...
        if (conferenceData.organizer_id !== currentUser.id) {
            return response.status(403).json({
                error: 'Only the conference organizer can delete this conference',
            });
        }
        ...
        return response
            .status(200)
            .json({ message: 'Conference deleted successfully' });
    } catch (error) {
